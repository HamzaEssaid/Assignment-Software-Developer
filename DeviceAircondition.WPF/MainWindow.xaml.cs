using DeviceAircondition.WPF.Models;
...
namespace DeviceAircondition.WPF
{

    public partial class MainWindow : Window
    {
        private readonly string _connect_url = "http://localhost:7113/api/devices/connect";
        private readonly string _connectionString = "Data Source=(LocalDB)\\MSSQLLocalDB;AttachDbFilename=C:\\Users\\gongm\\source\\repos\\Assignment-Software-Developer\\DeviceAircondition.WPF\\Data\\device.aircondition.bedroom_db.mdf;Integrated Security=True;Connect Timeout=30";
        private DeviceClient _deviceClient;
        private DeviceInfo _deviceInfo;

        private string _deviceId = "";
        private bool _airconState = false;
        private bool _airconPrevState = false;
        ...
        private async Task SetupAsync()
        {
            ...
            if (string.IsNullOrEmpty(_deviceId))
            {
                ...
                await conn.ExecuteAsync("INSERT INTO DeviceInfo (DeviceId,DeviceName,DeviceType,Location,Owner) VALUES (@DeviceId, @DeviceName, @DeviceType, @Location, @Owner)", new { DeviceId = _deviceId, DeviceName = "WPF DeviceAircondition", DeviceType = "aircondition", Location = "bedroom", Owner = "Gong Moonphruk" });
            }
            ...
            twinCollection["aircondition"] = _airconState;
        ...
        private async Task SendMessageAsync()
        {
            while (true)
            {
                if (_connected)
                {
                    if (_airconState != _airconPrevState)
                    {
                        _airconPrevState = _airconState;

                        // d2c
                        var json = JsonConvert.SerializeObject(new { airconditionState = _airconState });
                        ...
                        var twinCollection = new TwinCollection();
                        twinCollection["fanState"] = _airconState;
                        await _deviceClient.UpdateReportedPropertiesAsync(twinCollection);
                    }
                }
